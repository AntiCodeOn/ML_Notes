...
def test_logC():
   for a in range(1, 20, 2):
      npt.assert_equal(vi.logC(np.array([[a]])), 0.0)

def test_logC2():
   npt.assert_allclose(vi.logC(np.array([[1., 2., 3.]])), 4.09434456222)
...
def test_psi_diff3():
   diff_psi = vi.psi_diff(np.array([[1., 2.],[2., 1.]]))
...
def test_exp_pi():
   ...
   p_pi = vi.exp_p_pi(param.reshape(1,2))
...
def test_exp_p_theta():
   ...
   p_theta = vi.exp_p_theta([[1., 2.],[2., 1.]])
   npt.assert_equal(1.,1.)

def test_exp_p_Z():
   r_nk = np.ones((5,2))
   p_Z = vi.exp_p_Z(r_nk, np.array([[1., 2.]]))
   expected = -10.
